...
class CommentPostgreSQLService implements CommentRepository {
  ...
  Future<Comment> add(Comment newComment) async {
    ...
    await _connectionWrapper.connection.query("INSERT INTO comments (user_id, comment_date, content, posts_id) VALUES (@userId, @commentDate, @content, @postId)", substitutionValues: {
    ...
    result = await _connectionWrapper.connection.query("SELECT id FROM comments WHERE user_id = @userId AND comment_date = @commentDate, content = @content, posts_id = @postId", substitutionValues: {
      "userId" : newComment.userId,
      "commentDate" : newComment.commentDate,
      "content" : newComment.content,
      "postId" : newComment.postId
  ...
  Future<Comment> find(int commentId) async {    
    ...
    var result = await _connectionWrapper.connection.query("SELECT user_id, comment_date, content, posts_id FROM comments WHERE user_id = $commentId");
  ...
  Future<List<Comment>> findAll() async {
    ...
    var rows = await _connectionWrapper.connection.query("SELECT id, user_id, comment_date, content, posts_id FROM comments");
  ...
  Future<List<Comment>> findByPostId(int postId) async {
    List<Comment> comments = [];
    var rows = await _connectionWrapper.connection.query("SELECT id, user_id, comment_date, content FROM comments WHERE posts_id = $postId");
  ...
  Future<Comment> update(Comment commentToUpdate) async {
    ...
    await _connectionWrapper.connection.query("UPDATE comments SET user_id = @userId, comment_date = @commentDate, content = @content, posts_id = @postId)", substitutionValues: {
